...
import { selectgenders } from '../../genders/genderSlice'
...
export const ProductList = ({isBannerVisible=true}) => {
...

    {
        ...
        <motion.div style={{position:"fixed",backgroundColor:"white",height:"100vh",padding:'1rem',overflowY:"scroll",width:is500?"100vw":"30rem",zIndex:500}}  variants={{show:{left:0},hide:{left:-500}}} initial={'hide'} transition={{ease:"easeInOut",duration:.7,type:"spring"}} animate={isProductFilterOpen===true?"show":"hide"}>
            ...
            <Stack mb={'5rem'}  sx={{scrollBehavior:"smooth",overflowY:"scroll"}}>
                        ...
                        <Typography variant='h4'>New arivals</Typography>
                    ...
                    <Stack rowGap={2} mt={4} >
                        <Typography sx={{cursor:"pointer"}} variant='body2'>Hoodies</Typography>
                        <Typography sx={{cursor:"pointer"}} variant='body2'>Sweatshirts</Typography>
                        <Typography sx={{cursor:"pointer"}} variant='body2'>Polo T-shirts</Typography>
                        <Typography sx={{cursor:"pointer"}} variant='body2'>Dresses</Typography>
                        <Typography sx={{cursor:"pointer"}} variant='body2'>Nighty Suits</Typography>
                    </Stack>

                    {/* gender filters */}
                    <Stack mt={2}>
                        <Accordion>
                            <AccordionSummary expandIcon={<AddIcon />}  aria-controls="gender-filters" id="gender-filters" >
                                    <Typography>genders</Typography>
                            </AccordionSummary>

                            <AccordionDetails sx={{p:0}}>
                                <FormGroup onChange={handlegenderFilters}>
                                    {
                                        genders?.map((gender)=>(
                                            <motion.div style={{width:"fit-content"}} whileHover={{x:5}} whileTap={{scale:0.9}}>
                                                <FormControlLabel sx={{ml:1}} control={<Checkbox whileHover={{scale:1.1}} />} label={gender.name} value={gender._id} />
                                            </motion.div>
                                        ))
                                    }
                                </FormGroup>
                    ...
                    <Grid gap={is700?1:2} container justifyContent={'center'} alignContent={'center'}>
                        {
                            products.map((product)=>(
                                <ProductCard key={product._id} id={product._id} title={product.title} thumbnail={product.thumbnail} gender={product.gender.name} price={product.price} handleAddRemoveFromWishlist={handleAddRemoveFromWishlist}/>
                            ))
                        }
                    </Grid>
