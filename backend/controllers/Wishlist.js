...
exports.create=async(req,res)=>{
    try {
        const created=await new Wishlist(req.body).populate({path:"product",populate:["gender"]})
        await created.save()
...
exports.getByUserId=async(req,res)=>{
    try {
        ...
        const result=await Wishlist.find({user:id}).skip(skip).limit(limit).populate({path:"product",populate:['gender']})
