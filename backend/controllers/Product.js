...
exports.getAll = async (req, res) => {
    try {
        ...
        const totalDocs=await Product.find(filter).sort(sort).populate("gender").countDocuments().exec()
        const results=await Product.find(filter).sort(sort).populate("gender").skip(skip).limit(limit).exec()
...
exports.getById=async(req,res)=>{
    try {
        const {id}=req.params
        const result=await Product.findById(id).populate("gender").populate("category")
...
exports.undeleteById=async(req,res)=>{
    try {
        const {id}=req.params
        const unDeleted=await Product.findByIdAndUpdate(id,{isDeleted:false},{new:true}).populate('gender')
...
exports.deleteById=async(req,res)=>{
    try {
        const {id}=req.params
        const deleted=await Product.findByIdAndUpdate(id,{isDeleted:true},{new:true}).populate("gender")
